...
# -*- coding: utf-8 -*-
"""
CIFAR-10 interpretable classification using prototypes.
Code modified from https://github.com/OscarcarLi/PrototypeDL
O. Li et al, Deep Learning for Case-based Reasoning through Prototypes:
...
local_seed_collection = np.random.randint(low=0, high=2**31-1, dtype='int32', size=(num_tries,))
best_ac = np.zeros((num_tries,))
...
save_step = 10            # how frequently do we save the model to disk
...
input_height = 32         # data input shape
input_width = input_height
n_input_channel = 3       # the number of color channels; for gray it's 1, for colored it's 3
input_size = input_height * input_width * n_input_channel   # the number of pixels in one input image
n_classes = 10

# Network Parameters
n_prototypes = 60         # the number of prototypes
n_layers = 4
...
for current_try in range(num_tries):
  ...
  model_folder = os.path.join(os.getcwd(), "saved_model", "cifar_model", "cifar_color_try" + str(current_try))
  ...
  with tf.Session(config=config) as sess:
      ...
      n_valid_batch = x_val.shape[0] // batch_size
      n_test_batch  = x_test.shape[0] // batch_size
      # Training cycle
      for epoch in range(training_epochs):
          ...
          for i in range(n_train_batch):
              ...
              elastic_batch_x = batch_elastic_transform_color(batch_x, sigma=sigma, alpha=alpha,
                                                            height=input_height, width=input_width,
                                                            channels=n_input_channel)
          ...
          # save best accuracy for this initialization
          if valid_ac > best_ac[current_try]:
            best_ac[current_try] = valid_ac
          ...
          if epoch % save_step == 0 or epoch == training_epochs - 1:
              ...
              encode_decode = sess.run(X_decoded,
                                       feed_dict={X: x_train[:examples_to_show]})

              # Compare original images with their reconstructions
              f, a = plt.subplots(2, examples_to_show, figsize=(examples_to_show, 2))
              for i in range(examples_to_show):
                  a[0][i].imshow(x_train[i].reshape(input_height, input_width, n_input_channel),
                                  cmap='gray',
                                  interpolation='none')
                  a[0][i].axis('off')
  ...
  # Write the best accuracies in separate file
  # the directory to save the model
  model_folder = os.path.join(os.getcwd(), "saved_model", "cifar_model", "cifar_color_overall_try_results")
  makedirs(model_folder)
  img_folder = os.path.join(model_folder, "img")
  makedirs(img_folder)
  model_filename = "cifar_cae"
  ...
  console_log = open(os.path.join(model_folder, "console_log.txt"), "w+")
  for current_try in range(num_tries):
    print_and_write("\tBest accuracy: {:.4f}".format(best_ac[current_try]), console_log)
