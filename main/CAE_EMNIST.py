...
# -*- coding: utf-8 -*-
"""
EMNIST interpretable classification using prototypes.
...
from data import load_data_serialize
from data_preprocessing import batch_elastic_transform

# Import EMNIST data in serialized 2-D format
data_file_path = 'EMNIST_data\\emnist-balanced.mat'
((training_images, training_labels), (val_images, val_labels), (testing_images, testing_labels), mapping, nb_classes, enc) = load_data_serialize(data_file_path,0.1)
...
model_folder = os.path.join(os.getcwd(), "saved_model", "emnist_model", "emnist_cae_3_proto_47")
makedirs(model_folder)
img_folder = os.path.join(model_folder, "img")
makedirs(img_folder)
model_filename = "emnist_cae_3_proto_47"
...
# Network Parameters
n_prototypes = 47         # the number of prototypes
n_layers = 4
...
with tf.Session(config=config) as sess:
    ...
    n_train_batch = training_images.shape[0] // batch_size
    n_valid_batch = val_images.shape[0] // batch_size
    n_test_batch = testing_images.shape[0] // batch_size
    # Training cycle
    for epoch in range(training_epochs):
        ...
        for i in range(n_train_batch):
            batch_x = training_images[i*batch_size:(i+1)*batch_size,:]
        ...
        for i in range(n_valid_batch):
            batch_x = val_images[i*batch_size:(i+1)*batch_size,:]
            batch_y = val_labels[i*batch_size:(i+1)*batch_size,:]
        ...
        if epoch % test_display_step == 0 or epoch == training_epochs - 1:
            ...
            for i in range(n_test_batch):
                batch_x = testing_images[i*batch_size:(i+1)*batch_size,:]
                batch_y = testing_labels[i*batch_size:(i+1)*batch_size,:]
        ...
        if epoch % save_step == 0 or epoch == training_epochs - 1:
            ...
            encode_decode = sess.run(X_decoded,
                                     feed_dict={X: training_images[:examples_to_show]})

            # Compare original images with their reconstructions
            f, a = plt.subplots(2, examples_to_show, figsize=(examples_to_show, 2))
            for i in range(examples_to_show):
                a[0][i].imshow(training_images[i].reshape(input_height, input_width),
                                cmap='gray',
                                interpolation='none')
                a[0][i].axis('off')
